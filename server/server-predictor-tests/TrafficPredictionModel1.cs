...
namespace PredictionServer
{
    class TrafficPredictionModel1
    {
        ...
        public TrafficPredictionModel1(DataSystem ds)
        {
            ...
            modelOptions["smrCars_ahead_data"] = 2;// 10.0;//
            modelOptions["smrCars_ahead"] = 2.0; //time that is to be predicted ahead (and returned by predict)
            modelOptions["smrCars_link_count"] = 240.0;// 20.0;
            modelOptions["smrCars_look_behind_prediction"] = 2.0;// 10.0;
            modelOptions["smrCars_look_behind"] = 4.0;// 30.0;
            modelOptions["smrCars_look_behind_prediction_self"] = 4.0;// 30.0;
            ...
            )
        {
          ...
          //  engine.OptimiseForVariables = dbnCars.Item2;
            ...
            int link_count = getIntOpt("Cars", "link_count");
            for (int h = 1; h <= link_count; ++h)// getIntOpt("Cars", "link_count"))
            {
