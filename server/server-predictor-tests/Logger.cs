...
namespace PredictionServer
{
    ...
    class Logger
    {
        ...
        private void writeToLogFile(string message, Boolean newLine)
        {
            string logPath = "log.txt";

            if (!File.Exists(logPath))
            {
                FileStream tmp1 = File.Create(logPath);
                tmp1.Close();
            }
        ...
        public void log(string message, bool new_line = true)
        {
            ...
            writeToLogFile(logMessage, new_line);     
        }
        ...
        public void log_ifn(bool condition, string message)
        {
            if (!condition)
            {
                this.log_warning("WARNING:" + message);
            }
        }
